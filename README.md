...
- **切换项目**: 以子菜单形式动态列出所有项目文件夹，当前项目前会显示选中标记。菜单顶部提供「新建项目...」选项，可直接创建新项目并自动切换。项目目录有变动时菜单会自动刷新。通过在您的 `base_save_path` 中创建文件夹或使用菜单中的「新建项目...」即可创建新项目。
...
- **重置为默认值**: 将所有设置恢复为程序默认值。如果已启用开机自启，会同时从注册表中移除开机自启项。已有的截图文件不受影响。
