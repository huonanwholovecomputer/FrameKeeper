...
def export_to_video(icon):
    ...
    if not images:
        messagebox.showwarning("无图片", "所选文件夹中没有找到图片文件。", parent=dialog_parent)
        dialog_parent.destroy()
        return
    ...
    if frame is None:
        # 【BUG修复】temp_window 不存在，父窗口应为 root 或 None
        messagebox.showerror("错误", f"无法读取第一帧图片: {first_image_path}", parent=root)
        return
    ...
    # ... (所有 progress_window 内的组件设置代码保持不变)
    ...
    # 【关键修复】在grab_set之前，强制tkinter绘制窗口
    ...
    def update_from_queue():
        ...
        try:
            while not update_queue.empty():
                ...
                # 【BUG修复】这里应该使用 current 和 total 变量
    ...
    def do_export(update_q):
        ...
        try:
            ...
            estimated_total_size_mb = len(images) * first_frame_size * 0.5 / (1024 * 1024) 
            ...
            for i, image_name in enumerate(images):
                ...
                remaining = (elapsed / (i + 1)) * (len(images) - (i + 1)) if i < len(images) - 1 else 0
                update_q.put((i + 1, len(images), remaining, estimated_total_size_mb))
