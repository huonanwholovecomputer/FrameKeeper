...
def do_export_optimized(images, save_path, frame_rate, width, height, update_q, cancel_flag, base_count=0, total_count=None):
    ...
    try:
        ...
        if cancel_flag.is_set():
            root.after(0, lambda: messagebox.showinfo("导出取消", "视频导出已被取消。", parent=root))  # 如果取消标志被设置，显示取消消息
            return  # 退出函数

        '''完成导出'''
        ffmpeg_process.stdin.close()  # 关闭 FFmpeg 的标准输入，表示没有更多数据
        ...
        def update_from_queue():
            ...
            try:
                while not update_queue.empty():
                    ...
                    if error_info:
                        run_in_main_thread(messagebox.showwarning, "处理警告", error_info, parent=progress_window)  # 如果有错误信息，显示警告对话框
            except queue.Empty:
                pass  # 捕获队列为空的异常
