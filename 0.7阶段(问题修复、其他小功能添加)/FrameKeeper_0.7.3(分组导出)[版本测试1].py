...
def encode_worker(frame_queue, ffmpeg_stdin, total_frames, update_q, cancel_flag, start_time, num_producers):
    processed_count = 0
    last_update = 0
    min_update_interval = 0.1  # 最小更新间隔100ms
    min_frame_update = max(1, total_frames // 200)  # 动态计算最小更新帧数
    ...
    try:
        while True:
            ...
            try:
                ...
                if (processed_count - last_update >= min_frame_update or 
                    current_time - last_update >= min_update_interval):
        ...
        if not cancel_flag.is_set():
            update_q.put((total_frames, total_frames, 0, "完成"))
        ...
        for i, img_path in enumerate(chunk):
            ...
            try:
                with Image.open(img_path) as img:
                    resolution = img.size
                local_count[resolution] = local_count.get(resolution, 0) + 1
                if update_q:  # 使用传入的 update_q
                    update_q.put(1)  # 每次处理一张图片发送进度 1
            except Exception:
                continue
        return local_count

    with concurrent.futures.ThreadPoolExecutor() as executor:
        chunk_size = max(10, len(images) // (os.cpu_count() or 4))
        chunks = [images[i:i + chunk_size] for i in range(0, len(images), chunk_size)]
        
        futures = []
        for chunk in chunks:
            futures.append(executor.submit(scan_chunk, chunk))
        
        for future in concurrent.futures.as_completed(futures):
            if cancel_flag.is_set():
                break
            chunk_result = future.result()
            for res, count in chunk_result.items():
                resolution_count[res] = resolution_count.get(res, 0) + count
    
    return resolution_count

# 修改：在show_scan_progress函数中添加进度更新机制
def show_scan_progress(images, progress_window, cancel_flag):
    progress_frame = ttk.Frame(progress_window, padding=15)
    progress_frame.pack(fill=tk.BOTH, expand=True)
    
    ttk.Label(progress_frame, text="正在扫描图片分辨率...", font=("Arial", 10, "bold")).pack(anchor="w")
    ...
    ttk.Label(status_frame, textvariable=percent_var, font=("Arial", 10)).pack(anchor="w")
    
    files_var = tk.StringVar(value=f"0 / {len(images)} 图片已扫描")
    ttk.Label(status_frame, textvariable=files_var, font=("Arial", 9)).pack(anchor="w")
    ...
    # 定义 update_queue
    update_queue = queue.Queue()  # 新增：定义队列用于进度更新
    ...
    def update_progress():
        try:
            while not update_queue.empty():
                increment = update_queue.get_nowait()
                nonlocal scanned_count
                scanned_count += increment
                progress_var.set(scanned_count)
                percent = min(100, int(scanned_count * 100 / total_images))
                percent_var.set(f"{percent}%")
                files_var.set(f"{scanned_count} / {total_images} 图片已扫描")
        except queue.Empty:
            pass
        ...
        if scanned_count >= total_images or cancel_flag.is_set():
            ...
            progress_window.after(50, update_progress)  # 使用更短的延迟提高响应速度
    
    progress_window.after(100, update_progress)
    return progress_var, percent_var, files_var, update_queue

# 添加导出完成处理器
def handle_export_complete(task_id, future):
    """导出完成回调处理"""
    try:
        # 清理线程池资源
        if task_id in export_thread_pool:
            _, executor = export_thread_pool.pop(task_id)
            executor.shutdown(wait=False)
        
        # 处理结果
        if future.exception():
            error = future.exception()
            run_in_main_thread(messagebox.showerror, 
                              f"导出错误({task_id})", 
                              f"分辨率组导出失败: {error}")
    except Exception as e:
        print(f"Error handling export complete: {e}")

# 新增函数：按分辨率组导出
# 修改后的do_export_group函数
def do_export_group(images, save_path, frame_rate, width, height):
    # 创建进度窗口（保持不变）
    progress_window = tk.Toplevel(root)
    progress_window.withdraw()  # 隐藏根窗口
    '''进度窗口'''
    ...
    task_var = tk.StringVar(value="状态: 等待执行队列中...")
    ttk.Label(task_frame, textvariable=task_var, font=("Arial", 9)).pack(anchor="w")

    cancel_flag = threading.Event()
    update_queue = queue.Queue()
    
    # 唯一任务ID
    task_id = f"{os.getpid()}_{time.time()}_{width}x{height}"
    
    # 添加任务取消功能
    def cancel_export():
        if messagebox.askyesno("确认取消", "确定要取消视频导出吗？", parent=progress_window):
            cancel_flag.set()
            task_var.set("状态: 取消请求已发送...")
            
            # 尝试从队列中移除任务
            if task_id in export_thread_pool:
                future, executor = export_thread_pool[task_id]
                future.cancel()
                executor.shutdown(wait=False)
            
            # 通知用户
            root.after(1000, progress_window.destroy)
    ...
    '''准备导出任务数据'''
    task_data = {
        "args": (images, save_path, frame_rate, width, height, update_queue, cancel_flag),
        "callback": lambda task_id, f: handle_export_complete(task_id, f)
    }
    
    # 添加到全局队列
    export_queue.put((task_id, task_data))
    task_var.set(f"状态: 队列中（位置：{export_queue.qsize()}）...")
    ...
    def update_from_queue():
        if not progress_window.winfo_exists():  # 窗口已被销毁
            return
            
        # 一次性处理所有更新消息
        updated = False
        while not update_queue.empty():
            current, total, remaining_time, msg = update_queue.get_nowait()
            
            if msg == "取消导出" or cancel_flag.is_set():
                task_var.set("状态: 已取消")
                root.after(1000, progress_window.destroy)
                return
            elif msg == "完成":
                task_var.set("状态: 导出完成")
                root.after(2000, progress_window.destroy)
                return
        ...
        # 如果没有更新但任务仍在进行中，继续检查
        if progress_window.winfo_exists() and not cancel_flag.is_set():
...
def export_to_video(icon=None):
    ...
    '''扫描所有图片的分辨率'''
    scan_cancel_flag = threading.Event()
    scan_progress_window = tk.Toplevel(root)
    scan_progress_window.title("扫描图片分辨率")
    scan_progress_window.geometry("400x300")
    scan_progress_window.resizable(False, False)
    scan_progress_window.protocol("WM_DELETE_WINDOW", lambda: None)

    # 修改：接收返回的update_queue
    progress_var, percent_var, files_var, scan_update_queue = show_scan_progress(images, scan_progress_window, scan_cancel_flag)

    # 更新扫描进度的函数
    def update_scan_progress(current, total):
        percent = int(current * 100 / total)
        percent_var.set(f"{percent}%")
        files_var.set(f"{current} / {total} 图片已扫描")
        progress_var.set(current)
        if current >= total or scan_cancel_flag.is_set():
    ...
    def scan_task():
        ...
        try:
            # 修改：传递更新队列
            resolution_count = scan_image_resolutions(images, scan_update_queue, scan_cancel_flag)
        finally:
            # 确保扫描完成时关闭窗口
    ...
    # 等待扫描窗口关闭
    root.wait_window(scan_progress_window)
    ...
    # 分析分辨率统计结果
    total_images = len(images)
    resolutions = sorted(resolution_count.items(), key=lambda x: x[1], reverse=True)
    main_resolution = resolutions[0][0] if resolutions else (0, 0)
    other_resolutions = [res for res, count in resolutions if count < total_images * 0.5]
    resolution_types = len(resolutions)
    
    # 如果存在多种分辨率且非主流分辨率占比>5%，提示用户
    if resolution_types > 1 and any(count < total_images * 0.5 for res, count in resolutions):
        msg = (
            f"检测到 {resolution_types} 种不同的图片分辨率:\n\n"
            f"主流分辨率 ({main_resolution[0]}x{main_resolution[1]}): {resolution_count.get(main_resolution, 0)} 张 ({resolution_count.get(main_resolution, 0)/total_images*100:.1f}%)\n"
        )
        
        for res, count in resolutions:
            if res != main_resolution:
                msg += f"其他分辨率 ({res[0]}x{res[1]}): {count} 张 ({count/total_images*100:.1f}%)\n"
        
        msg += "\n导出为单一视频可能导致乱码。是否按分辨率分组导出为多个视频？"
        
        if messagebox.askyesno("分辨率警告", msg, parent=root):
            # 按分辨率分组图片
            grouped_images = {}
            for img_path in images:
                try:
                    with Image.open(img_path) as img:
                        res = img.size
                    if res not in grouped_images:
                        grouped_images[res] = []
                    grouped_images[res].append(img_path)
                except Exception:
                    continue
            
            # 修改点开始：按分辨率占比从大到小排序分组
            # 按组内图片数量从多到少排序
            ...
            # 依次处理每个组（按分辨率占比从大到小）
            for resolution, img_list in sorted_groups:
                width, height = resolution
                base_name, ext = os.path.splitext(save_path)
                group_save_path = f"{base_name}_{width}x{height}{ext}"
                
                # 获取用户设置的帧率
                frame_rate = simpledialog.askinteger(
                    f"分辨率: {width}x{height}",
                    f"请输入 {width}x{height} 分辨率视频的帧率 (FPS):",
                    initialvalue=30,
                    minvalue=1,
                    maxvalue=60,
                    parent=root
                ...
                if not frame_rate:
                    continue  # 用户取消跳过此分辨率组
                
                # 导出该分辨率组的视频
                progress_window = tk.Toplevel(root)  # 为当前组创建专属进度窗口
                progress_window.title(f"正在导出 {width}x{height} 视频...")
                
                # 调用原有导出函数（确保导出完成后才继续下一个组）
                task_id = do_export_group(
                    img_list, 
                    group_save_path, 
                    frame_rate, 
                    width, 
    ...
    progress_window = tk.Toplevel(root)  # 创建进度窗口作为根窗口的子窗口
    progress_window.title("正在导出视频...")  # 设置窗口标题
    progress_window.geometry("400x230")  # 设置固定窗口大小
    progress_window.resizable(False, False)  # 禁止调整窗口大小
    progress_window.protocol("WM_DELETE_WINDOW", lambda: None)  # 禁用窗口关闭按钮
    ...
    def cancel_export():
        if messagebox.askyesno("确认取消", "你确定要取消视频导出吗？", parent=progress_window):
            cancel_flag.set()
            progress_window.destroy()  # 立即销毁进度窗口
    ...
    # 定义函数：UI更新循环
    def update_from_queue():
        if not progress_window.winfo_exists():  # 检查窗口是否存在
            return
        ...
        # 如果没有更新但已处理完毕
        if progress_var.get() < len(images) and not cancel_flag.is_set():
            progress_window.destroy()
...
# 全局导出Queue Manager
export_queue = queue.Queue(maxsize=3)  # 最多同时处理3个导出任务
export_thread_pool = {}  # 跟踪活动的导出任务
