...
# 定义函数：创建 FFmpeg 子进程用于视频编码
def create_ffmpeg_writer(output_path, width, height, fps, bitrate):
    '''确保 ffmpeg 可执行文件存在'''
    if not shutil.which('ffmpeg'):
        messagebox.showerror("错误", "找不到 FFmpeg。请确保 FFmpeg 已安装并已添加到系统 PATH 环境变量中。")
    ...
    while True:
        ...
        try:
            ...
            if frame is None:  # 如果图像读取失败
                messagebox.showerror(f"无法读取图片: {image_path}")  # 抛出IO错误
...
def do_export_optimized(images, save_path, frame_rate, width, height, update_q, cancel_flag):
    ...
    try:
        ...
        if ffmpeg_process.returncode != 0:
            messagebox.showerror(f"FFmpeg 编码失败，返回码: {ffmpeg_process.returncode}。请检查控制台输出。")
...
def export_to_video(icon=None):
    ...
    try:
        with open(images[0], 'rb') as f:
            ...
            first_image = cv2.imdecode(img_np, cv2.IMREAD_COLOR)
        if first_image is None:  # 如果读取失败
            messagebox.showerror("错误", f"无法读取第一帧图片: {images[0]}")
...
def switch_project(project_name):
    new_project_path = os.path.join(config.base_save_path, project_name)
    ...
    messagebox.showinfo("切换成功", f"已切换到项目: {project_name}")
...
def project_menu_items():
    ...
    def make_project_item(project_name):
        return item(project_name, lambda: switch_project(project_name), checked=lambda item: config.project_name == project_name)
    project_menu_items = [make_project_item(name) for name in project_names]
    return pystray.Menu(*project_menu_items)

# 更新托盘菜单
def update_menu(icon):
    status_text = "运行中" if config.is_running else "已停止"
    start_stop_text = "停止截图" if config.is_running else "开始截图"
    start_stop_action = lambda: stop_screenshotting(icon) if config.is_running else start_screenshotting(icon)

    # 如果没有项目则添加提示项
    if not project_menu_items:
        project_menu_items.append(item("(无项目)", None, enabled=False))
