...
def clean_nonstandard_frame(root_directory):
    confirm = messagebox.askyesno("确认清理", f"即将清理冗余截图，截图时间间隔为 {config.interval} 秒\n删除后不可恢复，是否继续?", icon='warning')  # 弹出确认对话框，询问用户是否继续清理操作
    ...
    for dirpath, dirnames, filenames in os.walk(root_directory):  # 遍历根目录及其所有子目录
        ...
        for time, filename in file_times[1:]:
            ...
            if delta < config.interval:  # 如果时间间隔小于10秒
        ...
        for filename in delete_list:
            ...
            file_log = f" - {filename}"
            log_content.append(file_log)
            try:
                os.remove(filepath)
                dir_deleted += 1
            except Exception as e:
                error_log = f"  删除失败 {filename}: {str(e)}"
                log_content.append(error_log)

        total_deleted += dir_deleted

    '''将日志写入桌面文件'''
    desktop_path = os.path.join(os.path.expanduser("~"), "Desktop")
    log_file = os.path.join(desktop_path, "详细信息.log")
    try:
        with open(log_file, 'w', encoding='utf-8') as f:
            f.write("\n".join(log_content))
    except Exception as e:
        messagebox.showerror("错误", f"无法写入日志文件: {str(e)}")
...
def open_settings_window(icon):
    ...
    ttk.Label(main_frame, text="保存路径:").grid(row=1, column=0, sticky="w", pady=5)  # 添加保存路径标签(第1行第0列)
    ...
    try:
        ...
        while producers_finished < num_producers and not cancel_flag.is_set():
            try:
                ...
                if (processed_count % max(1, total_frames // 100) == 0 or current_time - last_update_time > 0.5):  # 更新进度条件：每处理约 1% 的帧或超过 0.5 秒未更新
                    elapsed = current_time - start_time  # 计算已用时间
                    remaining = (elapsed / processed_count) * (total_frames - processed_count) if processed_count > 0 else 0  # 计算剩余时间（基于当前处理速度）
...
def do_export_optimized(images, save_path, frame_rate, width, height, update_q, cancel_flag):
    ...
    try:
        ...
        num_reader_threads = max(1, min(num_cores - 1, 4))  # 限制最大线程数：最少1个，最多4个或CPU核心数-1
        
        '''基于图片数量动态设置队列大小'''
        if len(images) < 100:  # 图片数量小于100
            max_queue_size = 10  # 小队列
        elif len(images) < 500:  # 图片数量100-500
            max_queue_size = 20  # 中等队列
        else:  # 图片数量大于500
            max_queue_size = 30  # 大队列

        '''创建队列'''
        path_queue = queue.Queue()  # 图片路径队列（无大小限制）
        frame_queue = queue.Queue(maxsize=max_queue_size)  # 帧队列（有大小限制）
        for img_path in images:  # 将所有图片路径放入队列
            path_queue.put(img_path)

        '''初始化视频编码器'''
        temp_output = os.path.join(tempfile.gettempdir(), f"temp_export_{os.getpid()}.mp4")  # 生成临时文件路径
        fourcc = cv2.VideoWriter_fourcc(*"mp4v")  # 使用MP4V编解码器
        video = cv2.VideoWriter(temp_output, fourcc, frame_rate, (width, height))  # 创建视频写入器
        if not video.isOpened():  # 检查视频写入器是否成功初始化
        ...
        '''消费者线程'''
        encoder_thread = threading.Thread(target=encode_worker, args=(frame_queue, video, len(images), update_q, cancel_flag, start_time, num_reader_threads), daemon=True)  # 设置为守护线程
        encoder_thread.start()  # 启动编码线程
        threads.append(encoder_thread)  # 添加到线程列表

        '''生产者线程'''
        for _ in range(num_reader_threads):  # 根据计算出的线程数创建生产者线程
            reader_thread = threading.Thread(target=read_and_decode_worker, args=(path_queue, frame_queue, cancel_flag, 95), daemon=True)  # 95%内存阈值 
            reader_thread.start()  # 启动读取线程
            threads.append(reader_thread)  # 添加到线程列表

        path_queue.join()  # 等待所有图片路径处理完成
        
        '''发送结束信号'''
        for _ in range(num_reader_threads):  # 向每个生产者线程发送结束信号
            path_queue.put(None)
            
        '''等待线程结束'''
        for t in threads:  # 等待所有线程完成
            t.join(timeout=5.0)  # 最多等待5秒
        if not exception_queue.empty():  # 检查是否有异常
            raise exception_queue.get()  # 抛出异常
        if cancel_flag.is_set():  # 检查是否被取消
            root.after(0, lambda: messagebox.showinfo("导出取消", "视频导出已被取消。", parent=root))
            return

        '''完成导出'''
        video.release()  # 释放视频资源
        shutil.move(temp_output, save_path)  # 将临时文件移动到目标位置
        final_size = os.path.getsize(save_path) / (1024 * 1024)  # 计算最终文件大小(MB)
        duration = len(images) / frame_rate  # 计算视频时长
        ...
        if video is not None:  # 确保视频写入器被释放
            video.release()
        if temp_output and os.path.exists(temp_output):  # 确保临时文件被删除
            try:
...
def export_to_video(icon=None):
    ...
    source_dir = filedialog.askdirectory(
        title="选择包含截图的文件夹",
        initialdir=config.base_save_path,  # 默认打开配置的保存路径
    ...
    save_path = filedialog.asksaveasfilename(
        ...
        title="保存视频文件",  # 对话框标题
        initialdir=config.base_save_path,  # 默认目录
        parent=dialog_parent
    )
    ...
    '''读取第一帧图片获取视频尺寸'''
    try:
        first_image = cv2.imread(images[0])  # 读取第一张图片
    ...
    def cancel_export():
        if messagebox.askyesno(
            "确认取消", 
            "你确定要取消视频导出吗？", 
            parent=progress_window  # 设置父窗口确保对话框居中
        ):
            cancel_flag.set()  # 设置取消标志，通知所有工作线程停止
    
    '''创建取消按钮区域'''
    button_frame = ttk.Frame(progress_frame)  # 按钮容器框架
    button_frame.pack(fill=tk.X, pady=(10, 0))  # 填充水平空间，上方留白10px
...
def update_menu(icon):
    ...
    icon.menu = pystray.Menu(item(f"状态: {status_text}", None, enabled=False), pystray.Menu.SEPARATOR, start_stop_item, item("清理冗余截图", lambda: clean_nonstandard_frame(config.base_save_path)), item("导出为视频", lambda: run_in_main_thread(export_to_video, icon)), item("设置", lambda: open_settings_window(icon)), pystray.Menu.SEPARATOR, item("退出", lambda: on_quit(icon)))  # 托盘菜单
